...
- 模式决策统一：`POWERX_PROVIDER_MODE` + `POWERX_PROXY` 必须可解释、可观测。
- 当 `POWERX_PROVIDER_MODE=local && POWERX_PROXY=1`：
  - 插件 IAM 走本地；
  - WS/能力出站走宿主；
...
## 5. 模式切换策略
- 配置项：`POWERX_PROVIDER_MODE`、`POWERX_PROXY`、`PX_GATEWAY_BASE_URL`、`PX_GATEWAY_API_KEY`、`POWERX_STS_CLIENT_ID`、`POWERX_STS_CLIENT_SECRET`
- `standalone`：允许本地运行，但事件与任务接口保持 framework 同构。
- `host`：通过 PowerX Framework 注入统一实现。
- 最小环境变量集：
  - local+proxy 必须：`POWERX_PROXY=1`、`POWERX_PROVIDER_MODE=local`、`PX_GATEWAY_BASE_URL`、`PX_GATEWAY_API_KEY`
  - host 必须：`POWERX_PROXY=1`、`POWERX_PROVIDER_MODE=delegated`、`PX_GATEWAY_BASE_URL`、`POWERX_STS_CLIENT_ID`、`POWERX_STS_CLIENT_SECRET`
