...
### User Story 4 - 后台会籍管理与客户会籍查看 (Priority: P2)

作为运营人员，我需要在后台查看会籍等级/权益配置，并在客户维度查看其会籍、权益与代币余额，以便运营与售后处理。

**Why this priority**: 订阅发放落地后必须有后台可视化入口，否则无法运营与排查问题。

**Independent Test**: 后台页面可访问并展示会籍/权益与客户会籍详情。

**Acceptance Scenarios**:

1. **Given** 会籍等级与权益已配置，**When** 进入后台会籍管理页面，**Then** 可查看会籍等级与权益列表。
2. **Given** 客户存在会籍/权益/代币余额，**When** 在客户详情中打开会籍页签，**Then** 能看到权益与代币。
...
- **FR-009**: 后台必须提供会籍管理入口，用于查看会籍等级与权益配置列表。
- **FR-010**: 后台必须在客户详情中展示会籍/权益/代币信息。
- **FR-011**: 后台必须支持手动发放/回收权益，以及调整代币余额（含审计记录）。
