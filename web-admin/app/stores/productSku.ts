...
const defaultConfig = () => {
  ...
  return (
    productSkuApiBase || "/_p/com.powerx.plugins.ecommerce/api/v1/products/skus"
  );
};

const ACTIVE_BULK_STATUSES: SkuBulkTaskStatus[] = [
  "pending",
  "approved",
  "running",
];
...
const normalizeListParams = (params?: SkuListParams): SkuListParams => {
  ...
  const normalized: SkuListParams = {
    ...
    pageSize:
      typeof safe.pageSize === "number" && safe.pageSize > 0
        ? safe.pageSize
        : 20,
  };
  ...
  actions: {
    async fetchList(params?: SkuListParams) {
      ...
      try {
        ...
        this.setItems(
          items as ProductSku[],
          typeof totalFromServer === "number" ? totalFromServer : items.length,
        );
        return this.items;
      } catch (error) {
        throw error;
    ...
    async saveChannelMapping(skuId: string, payload: SkuChannelMappingPayload) {
      ...
      const filtered = current.filter(
        (item) => item.channelCode !== mapping.channelCode,
      );
      this.channelMappings[skuId] = [...filtered, mapping].sort((a, b) =>
        a.channelCode.localeCompare(b.channelCode),
      );
      return mapping;
    },
    async publishChannelMapping(
      skuId: string,
      payload: SkuChannelPublishPayload,
    ) {
      if (!skuId) {
        throw new Error("skuId is required");
      }
    ...
    applyBulkTaskEvent(event: any) {
      ...
      const taskId = String(
        payload.taskId ?? payload.task_id ?? payload.id ?? "",
      ).trim();
      if (!taskId) return;

      const current = this.bulkTasks[taskId];
      const nextStatus = normalizeBulkTaskEventStatus(
        payload.status ?? payload.state ?? current?.status,
      );
      ...
      const merged: SkuBulkTask = {
        ...
        taskType: (payload.taskType ??
          payload.task_type ??
          current?.taskType ??
          "import") as SkuBulkTask["taskType"],
        ...
        approvalState: (payload.approvalState ??
          ...
          current?.approvalState) as SkuBulkTask["approvalState"],
        approvalReason:
        ...
        submittedBy:
          payload.submittedBy ?? payload.submitted_by ?? current?.submittedBy,
        approvedBy:
          payload.approvedBy ?? payload.approved_by ?? current?.approvedBy,
        approvedAt:
          payload.approvedAt ?? payload.approved_at ?? current?.approvedAt,
        errorReportUrl:
          payload.errorReportUrl ??
          payload.error_report ??
          current?.errorReportUrl,
        stats: payload.stats ?? current?.stats,
        createdAt:
          payload.createdAt ?? payload.created_at ?? current?.createdAt ?? now,
    ...
    async fetchGeneratorCandidates(
      spuId: string,
      request: SkuGeneratorRequest,
    ) {
      const api = useSkuApi();
      this.generatorLoading = true;
      try {
        const resp = await api.generate(spuId, request);
        this.generatorCandidates = (resp?.candidates ?? []).map(
          normalizeCandidate,
        );
    ...
    async createSkus(payload: SkuUpsertRequest): Promise<SkuUpsertResult> {
      ...
      try {
        ...
        const result = normalizeUpsertResult(
          await api.create(serverPayload as any),
        );
        if (result?.created) {
    ...
    async refreshActiveBulkTasks() {
      const pendingTasks = Object.values(this.bulkTasks).filter((task) =>
        isActiveBulkTask(task),
      );
...
function normalizeCandidate(candidate: any): SkuGeneratorCandidate {
  ...
  return {
    specs,
    previewSkuCode:
      candidate?.preview_sku_code ?? candidate?.previewSkuCode ?? "",
    ...
    conflictReasons:
      candidate?.conflict_reasons ?? candidate?.conflictReasons ?? [],
  };
}
...
function normalizeBulkTaskEventStatus(raw: any): SkuBulkTaskStatus {
  const normalized = String(raw ?? "pending")
    .trim()
    .toLowerCase();
  switch (normalized) {
    case "success":
...
function isActiveBulkTask(task?: SkuBulkTask) {
  ...
  if (
    ...
    (task.approvalState === "pending" || !task.approvalState)
  ) {
    return true;
  }
  return false;
...
function normalizeBulkTaskResponse(task: any): SkuBulkTask {
  if (!task) {
    return {
      ...
      status: "pending",
      approvalRequired: Boolean(
        task?.approval_required ?? task?.approvalRequired,
      ),
    };
  }
  return {
...
function normalizeInventorySnapshot(raw: any): SkuInventorySnapshot {
  const warehouses = (raw?.warehouses ?? []).map((item: any) =>
    normalizeInventoryWarehouse(item),
  );
  ...
  return {
    ...
    summary,
    lastSyncedAt:
      raw?.last_synced_at ?? raw?.lastSyncedAt ?? summary.lastSyncedAt,
    isStale: Boolean(raw?.is_stale ?? raw?.isStale ?? false),
  };
}

function normalizeInventoryWarehouse(
  raw: any,
  fallbackId = "",
): SkuInventoryWarehouse {
  if (!raw) {
