  ...
  >
    ...
    <template #footer>
      <div class="flex gap-3">
        ...
        <UButton color="primary" :loading="loading" @click="submit">创建属性</UButton>
      </div>
    </template>
  </UModal>
</template>
...
const props = defineProps<{ open?: boolean, attributeToCopy?: any }>();
...
const title = computed(() => props.attributeToCopy ? '复制属性' : '新建属性');
const description = computed(() => props.attributeToCopy ? '基于现有属性创建一个新属性' : '创建一个新的商品属性');
...
watch(() => props.open, (newVal) => {
  if (newVal) {
    if (props.attributeToCopy) {
      // Copy from existing attribute
      formState.name = props.attributeToCopy.name + ' (副本)';
      formState.code = props.attributeToCopy.code + '_copy';
      formState.scope = props.attributeToCopy.scope;
      formState.status = props.attributeToCopy.status;
      formState.dataType = props.attributeToCopy.dataType;
      formState.isSearchable = props.attributeToCopy.isSearchable;
      formState.isAggregatable = props.attributeToCopy.isAggregatable;
      formState.isSortable = props.attributeToCopy.isSortable;
      formState.searchWeight = props.attributeToCopy.searchWeight;
      formState.validation = { ...props.attributeToCopy.validation };
    } else {
...
async function submit() {
  ...
  try {
    ...
    emit('created', newAttribute);
    close({ action: 'create', attribute: newAttribute });
  } catch (error) {
