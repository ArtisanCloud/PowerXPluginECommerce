  ...
  >
    ...
    <template #footer>
      <div class="flex gap-3">
        ...
        <UButton color="primary" :loading="loading" @click="submit">创建规格</UButton>
      </div>
    </template>
  </UModal>
</template>
...
const props = defineProps<{ open?: boolean, specificationToCopy?: any }>();
const emit = defineEmits<{
  "update:open": [boolean];
  created: [specification: any];
  close: [payload: any];
}>();
...
const title = computed(() => props.specificationToCopy ? '复制规格' : '新建规格');
const description = computed(() => props.specificationToCopy ? '基于现有规格创建一个新规格' : '创建一个新的商品规格');
...
const formState = reactive({
  ...
  validation: {
    ...
    minLength: null,
    maxLength: null,
    enumOptions: [{ value: 'Option 1', alias: '', sort: 1 }]
  },
  isSearchable: true,
...
function resetForm() {
  ...
  formState.dataType = 'text';
  formState.isSearchable = true;
  formState.isAggregatable = false;
  formState.isSortable = false;
  formState.searchWeight = 5;
...
watch(() => props.open, (newVal) => {
  if (newVal) {
    if (props.specificationToCopy) {
      formState.name = props.specificationToCopy.name + ' (副本)';
      formState.code = props.specificationToCopy.code + '_copy';
      formState.scope = props.specificationToCopy.scope;
      formState.status = props.specificationToCopy.status;
      formState.dataType = props.specificationToCopy.dataType;
      formState.isSearchable = props.specificationToCopy.isSearchable;
      formState.isAggregatable = props.specificationToCopy.isAggregatable;
      formState.isSortable = props.specificationToCopy.isSortable;
      formState.searchWeight = props.specificationToCopy.searchWeight;
      formState.validation = { ...props.specificationToCopy.validation };
    } else {
      resetForm();
    }
  }
...
async function submit() {
  ...
  try {
    ...
    emit('created', newSpecification);
    close({ action: 'create', specification: newSpecification });
  } catch (error) {
