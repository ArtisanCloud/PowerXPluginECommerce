...
const title = computed(() => {
  return props.mode === 'batch' ? '批量调整积分' : 
         form.value.type === 'add' ? '赠送积分' : '扣减积分';
});

const description = computed(() => {
  return props.mode === 'batch' ? '批量为多个客户账户增加或扣减积分' :
         form.value.type === 'add' ? '为客户账户增加积分' : '为客户账户扣减积分';
});
