...
const props = defineProps<{ open?: boolean, template: any, availableAttributeGroups: any[], categoryOptions: any[] }>();
...
watch(() => props.template, (newTemplate) => {
  if (newTemplate) {
    formState.id = newTemplate.id;
    formState.name = newTemplate.name;
    formState.code = newTemplate.code;
    formState.categoryId = newTemplate.categoryId || newTemplate.category;
    formState.status = newTemplate.status;
    formState.sortOrder = newTemplate.sortOrder;
    formState.description = newTemplate.description;
    formState.attributeGroups = JSON.parse(JSON.stringify(newTemplate.attributeGroups || []));
    formState.attributeGroupIds = (newTemplate.attributeGroups || []).map((g:any) => g.id);
  }
...
async function submit() {
  ...
  try {
    ...
    close({ action: 'update', template: updatedTemplate });
  } catch (error) {
    console.error("Failed to update category template:", error);
  } finally {
    loading.value = false;
  }
}
