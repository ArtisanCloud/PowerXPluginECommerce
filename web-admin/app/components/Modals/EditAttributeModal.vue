...
const props = defineProps<{ open?: boolean, attribute: any }>();
...
watch(() => props.attribute, (newAttr) => {
  if (newAttr) {
    formState.id = newAttr.id;
    formState.name = newAttr.name;
    formState.code = newAttr.code;
    formState.scope = newAttr.scope;
    formState.status = newAttr.status;
    formState.dataType = newAttr.dataType;
    formState.isSearchable = newAttr.isSearchable;
    formState.isAggregatable = newAttr.isAggregatable;
    formState.isSortable = newAttr.isSortable;
    formState.searchWeight = newAttr.searchWeight;
    // Ensure validation object and its properties exist before assigning
    formState.validation = {
      pattern: newAttr.validation?.pattern || '',
      minLength: newAttr.validation?.minLength || null,
      maxLength: newAttr.validation?.maxLength || null,
      enumOptions: newAttr.validation?.enumOptions ? JSON.parse(JSON.stringify(newAttr.validation.enumOptions)) : []
    };
  }
...
async function submit() {
  ...
  try {
    ...
    const updatedAttribute = { ...formState };
    emit('updated', updatedAttribute);
    close({ action: 'update', attribute: updatedAttribute });
  } catch (error) {
