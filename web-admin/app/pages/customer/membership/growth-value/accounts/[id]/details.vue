  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between gap-3">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">成长值明细</h1>
          <p class="text-sm text-gray-500 dark:text-gray-400">展示成长值历史调整记录</p>
        </div>
      </div>
    ...
    <UCard>
      <template #header>
        <h2 class="text-base font-semibold text-gray-900 dark:text-white">成长值记录</h2>
      </template>
      ...
      <UAlert v-if="!loading && !records.length" color="gray" class="mt-4">暂无成长值记录</UAlert>
    </UCard>
  </div>
</template>
...
const loadData = async () => {
  ...
  try {
    const [customer, tokenBalances, txnResp] = await Promise.all([
      ...
      membershipApi.listTokenTransactions({ customerId: accountId, tokenCode: 'growth_value', page: 1, pageSize: 100 }),
    ])
    ...
    account.value = {
      ...
      balance: Number(growthToken?.balance ?? snapshot?.growthValue ?? customer?.growthValue ?? 0) || 0,
    }
...
const exportData = () => {
  ...
  a.href = url
  a.download = `growth-transactions-${accountId}.csv`
  a.click()
