      ...
      <template #body>
        <div v-if="currentProduct" class="space-y-6 p-6">
            ...
            <div class="lg:col-span-2 space-y-6">
              ...
              <div class="grid grid-cols-2 gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <div class="flex justify-between">
                  <span class="text-gray-600 dark:text-gray-400">{{ $t('product.productId') }}:</span>
                  <span class="font-medium">{{ currentProduct.id }}</span>
                </div>
                <div class="flex justify-between">
          ...
          <div>
            ...
            <div class="prose dark:prose-invert max-w-none bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
...
const products = ref<Product[]>([
  ...
  },
  {
    ...
    status: "active",
    published: true,
    description: "三星旗舰手机，搭载骁龙8 Gen 2芯片，拥有出色的摄像系统和流畅的性能。",
    images: ["https://via.placeholder.com/300x300/ffff00/000000"],
    attributes: [
      { name: "型号", value: "SM-S9110" },
      { name: "颜色", value: "幻影黑, 奶油白, 紫色" },
  ...
  },
  {
    ...
    status: "active",
    published: true,
    description: "华为旗舰手机，搭载骁龙8+ Gen 1芯片，拥有徕卡摄像系统和昆仑玻璃。",
    images: ["https://via.placeholder.com/300x300/00ffff/000000"],
    attributes: [
      { name: "型号", value: "Mate50Pro" },
      { name: "颜色", value: "曜金黑, 翡冷翠, 流光紫" },
      { name: "存储容量", value: "256GB, 512GB" },
      { name: "屏幕尺寸", value: "6.74英寸" },
      { name: "处理器", value: "骁龙8+ Gen 1" }
    ]
  }
]);
...
const columns = computed<TableColumn<Product>[]>(() => [
  { accessorKey: "id", header: t("product.productId") },
...
const batchPublish = () => {
  ...
  alert(`已发布 ${selectedProducts.value.length} 个商品`);
  selectedProducts.value = [];
...
const batchUnpublish = () => {
  ...
  alert(`已取消发布 ${selectedProducts.value.length} 个商品`);
  selectedProducts.value = [];
  selectAll.value = false;
  // 这里应该调用API批量更新商品状态
};
...
const batchEnable = () => {
  ...
  alert(`已启用 ${selectedProducts.value.length} 个商品`);
  selectedProducts.value = [];
  selectAll.value = false;
  // 这里应该调用API批量更新商品状态
};
...
const batchDisable = () => {
  ...
  alert(`已禁用 ${selectedProducts.value.length} 个商品`);
  selectedProducts.value = [];
  selectAll.value = false;
  // 这里应该调用API批量更新商品状态
};
