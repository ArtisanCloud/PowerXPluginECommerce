...
func TestPromotionSnapshot_IsImmutableAfterCampaignChange(t *testing.T) {
	...
	now := time.Now().UTC()
	seedPromotion(t, db, promotionmodel.Campaign{ID: "promo-immutable", TenantUUID: tenant, Code: "IMM", Name: "不可变", PromotionType: promotionmodel.TypeAmountOff, ConditionRule: mustJSON(map[string]any{"min_order_amount_minor": 1000}), ScopeRule: mustJSON(map[string]any{"scope_type": "all"}), ActionRule: mustJSON(map[string]any{"discount_amount_minor": 100}), StackingRule: mustJSON(map[string]any{"priority": 100, "stackable": true, "stackable_with_coupon": true}), ValidFrom: now.Add(-time.Hour), ValidTo: now.Add(time.Hour), Status: promotionmodel.StatusActive})
	svc := NewService(&app.Deps{DB: db})
	created, err := svc.CreateOrder(ctx, tenant, adminID, "promo-idem-4", CreateOrderRequest{CustomerID: customerID, Channel: "official", ShippingAddress: &ShippingAddress{RecipientName: "张三", RecipientPhone: "13800138000", Address1: "北京"}, Items: []CreateOrderItemInput{{SKUID: skuID, Qty: 1}}})
	require.NoError(t, err)
	require.NoError(t, db.Exec(`UPDATE promotion_campaigns SET action_rule = ? WHERE id = ?`, string(mustJSON(map[string]any{"discount_amount_minor": 9999})), "promo-immutable").Error)
