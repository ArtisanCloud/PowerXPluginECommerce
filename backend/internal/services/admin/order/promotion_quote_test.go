...
func TestServiceCreateOrder_AppliesPromotion(t *testing.T) {
	...
	seedPromotion(t, db, promotionmodel.Campaign{
		ID: "promo-order", TenantUUID: tenant, Code: "ORDER1000", Name: "满减", PromotionType: promotionmodel.TypeAmountOff,
		...
		StackingRule:  mustJSON(map[string]any{"priority": 100, "stackable": true, "stackable_with_coupon": true}),
		ValidFrom:     now.Add(-time.Hour), ValidTo: now.Add(time.Hour), Status: promotionmodel.StatusActive,
	})
	svc := NewService(&app.Deps{DB: db})
	out, err := svc.CreateOrder(ctx, tenant, adminID, "promo-idem-1", CreateOrderRequest{
		CustomerID: customerID, Channel: "official",
	...
	require.Equal(t, int64(100), out.Promotion.PromotionDiscountMinor)
	require.Equal(t, int64(19800), out.Amounts.Total)
}
