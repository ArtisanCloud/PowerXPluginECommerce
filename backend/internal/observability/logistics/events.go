...
const (
	...
	eventMsg   = "logistics_event"
	auditMsg   = "logistics_audit_event"
)

// Emitter emits structured logistics observability events.
type Emitter struct {
	entry *logrus.Entry
}

// Event describes a domain event emitted by logistics services.
type Event struct {
	Action     string
	TenantUUID string
	RequestID  string
