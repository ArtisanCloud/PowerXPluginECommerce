...
- Go 1.24（backend）、TypeScript 5.9 + Nuxt 4（web-admin） + Gin、GORM(Postgres)、PowerX plugin framework、Nuxt UI 3.3.x、Pinia (011-fulfillment-logistics)
- PostgreSQL（schema `powerx_plugin_base`），可选 Redis 用于异步任务/事件缓存 (011-fulfillment-logistics)
- Go 1.24（backend）、TypeScript 4.x + Nuxt 4（web-admin） + Gin、GORM(Postgres)、PowerX plugin framework、Nuxt UI 3.3.x、Pinia (012-after-sales-rma)
- Backend Go 1.24；Frontend TypeScript 5.9 + Nuxt 4 + Gin, GORM(Postgres), PowerX plugin framework, taskcenter；Nuxt UI 3.3.x + Pinia (013-subscription-reconciliation)
- PostgreSQL（schema `powerx_plugin_base`），可选 Redis 仅用于任务去重缓存 (013-subscription-reconciliation)
- Go 1.24（backend）, TypeScript 5.9 + Nuxt 4（web-admin, Node 20） + Gin, GORM (Postgres), PowerX plugin framework, Nuxt UI 3.3.x, Pinia (015-pricing-promotions-engine)
- PostgreSQL (`powerx_plugin_base` schema), no required Redis dependency for MVP (015-pricing-promotions-engine)
